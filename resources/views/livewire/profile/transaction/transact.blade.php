<div>
    <div data-v-547979d4="" class="tab-pane show fade active">
        <div data-v-547979d4="">
            <div data-v-70ad3292="" class="account-table-container">
                <table data-v-483e03c5="" data-v-70ad3292="" aria-busy="false" aria-colcount="5" aria-rowcount="-1" class="table b-table table-hover table-dark transactions-table thead-no-border thead-text-normal card-background-color" sort-direction="asc" id="__BVID__120_">
                        ...
                        <tr aria-rowindex="{{ $loop->index }}">
                            ...
                            <td aria-colindex="4" class="blz-xs-35 blz-md-15 pl-1 pr-3 pl-md-4 align-middle text-left text-white blz-text-md-white-70">
                                <div data-v-70ad3292="" class="text-nowrap d-flex align-items-center justify-content-end justify-content-md-start">
                                    @if($item->service === 'withdraw')
                                        <span data-v-70ad3292="" title="@lang('account.status_'.$item->status)"
                                              class="text-truncate blz-text-capitalize-first blz-mw-100px">@lang('account.status_'.$item->status)</span>
                                    @else
                                        <span data-v-70ad3292="" title="@lang('payment.payment_status_'.$item->status)" class="text-truncate blz-text-capitalize-first blz-mw-100px">@lang('payment.payment_status_'.$item->status)</span>
                            ...
                            <td aria-colindex="5" class="d-none d-md-table-cell blz-md-15 align-middle">
                                @if($item->order_id)
                                    <a data-v-70ad3292="" href="{{ route('transactions.view', [$item->order_id, $item->id]) }}"
                                       class="d-none
                        d-md-block">
                                        EU{{$item->order_id}}
                                    </a>
            ...
            {{ $transact->links('paginate.transact') }}
        </div>
    </div>
</div>

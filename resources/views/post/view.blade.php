<x-guest-layout>
    ...
    @push('css')
        @vite(['resources/css/3.css', 'resources/css/news.css'])
    @endpush

    @push('scripts')
        <script src="{{ asset('static/runtime.7c6b2c574cb255b04c4f.js') }}"></script>
        <script src="{{ asset('static/responsive-blogs.499aff8da07ec27181f0.js') }}"></script>
        <script src="{{ asset('static/vendor.148b142df29c0644a491.js') }}"></script>
        <script src="{{ asset('static/3.d15a3fea6c61801c0cfa.js') }}"></script>
        <script src="{{ asset('static/news.bfe72acc74860520c064.js') }}"></script>
    @endpush

    <article>
        <header>
            <div class="Pane Pane--underSiteNav Pane--fadeBottom bordered" data-url="{{ asset($post->image) }}" queryselectoralways="31">
                <div class="Pane-bg" style="background-color:#000000;background-image:url({{ asset($post->image) }});">
                ...
                <div class="Pane-content">
                    ...
                    <div class="contain-max">
                        <h1 class="margin-none font-title-large-onDark">{{ $post->getTranslation('name', App()->getLocale()) }}</h1>
                        ...
                        <div class="List">
                            <div class="font-bliz-light-small-beige List-item gutter-tiny" media-medium="List-item gutter-tiny" queryselectoralways="0" media-original="font-bliz-light-small-beige">
                                <div class="Content">
                                    <a href="#">{{ Str::title($post->author->name) }}</a>,
                                    {!! $post->created_at !!}
                                </div>
                            </div>
                    ...
                    <div class="flex flex-items-center margin-bottom-normal">
                        <div class="SocialButtons font-none" queryselectoralways="43">
                           ...
                           <span class="Icon Icon--social-twitter SocialButtons-icon SocialButtons-icon--twitter">
                            ...
                            <div class="SocialButtons-button">
                                <a class="SocialButtons-link font-size-xSmall SocialButtons-link--vkontakte" href="javascript:void(0);" data-url="https://vkontakte.ru/share.php?url={{ route('post.show', [$post->id, $post->slug]) }}" data-popup-height="280" data-popup-width="550" data-analytics="sns-share" data-analytics-placement="News - vkontakte">
